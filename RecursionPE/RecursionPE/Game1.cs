...
namespace RecursionPE
{
    ...
    public class Game1 : Game
    {
        ...
        {
            if (width <=103 || height <= 103)
            {
                return DrawRecursiveThing(x, y, width, height, color);
            }
            else
            {
                ...
                {
                    DrawRecursiveThing(x + image.Width/2, y + image.Height/2, image.Width / 2, image.Height / 2, Color.Green);
                }
                if (color == Color.Green)
                {
                    DrawRecursiveThing(x+image.Width/2, y+image.Height/2, image.Width/2,image.Height/2, Color.White);
                }
            }
            return 1;
        }
